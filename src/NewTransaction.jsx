        ...
        <div>
          <form
            ...
            onSubmit={(e) => {
              ...
              if (validateInput()) {
                fetch(
                  `/api/m=itemdb_oldschool/api/catalogue/items.json?category=1&alpha=${formatItemName(
                    formItemName.toLowerCase(),
                  )}&page=1`,
                )
          ...
          >
            ...
            <div className="inputs">
              <input
                name="itemName"
                value={formItemName}
                onChange={({ target }) => setFormItemName(target.value)}
              />
              <input
                name="quantity"
                value={formItemQuantity}
                onChange={({ target }) => setFormItemQuantity(target.value)}
              />
              <input
                name="buyPrice"
                value={formItemBuyPrice}
                onChange={({ target }) => setFormItemBuyPrice(target.value)}
              />
            </div>
