...
export const Transaction = ({
  ...
  const doAction = () => {
    if (sellPrice > 0) {
      setProfit(
        Math.floor((sellPrice - buyPrice) * quantity - quantity * sellPrice * 0.01).toLocaleString(
          'en-US',
        ),
      )
  ...
  return (
    <div className={styles.transaction}>
      <div className={styles.info}>
        ...
        {!sold ? (
          <>
            ...
            <input
              id={styles['sellPriceField']}
              autoComplete="off"
              name="sellPrice"
              value={sellPrice}
              onChange={({ target }) => setSellPrice(target.value)}
            ></input>
          </>
        ) : (
          <>
      ...
      {!sold ? (
        <button id={styles['sellButton']} onClick={doAction}>
          Mark as sold
        </button>
      ) : null}
      <button
